...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    else setError('Invalid email or password. Use your created account or the demo account admin@localhost.ac / react123');
  };
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: '40px' }}>
        <div style={{ width: '100%', maxWidth: '380px', animation: 'fadeUp 0.4s ease both' }}>
          ...
          <form onSubmit={handleSubmit}>
            {successMessage && (
              <div style={{ background: 'rgba(45,125,70,0.08)', border: '1px solid var(--success)', borderRadius: '8px', padding: '11px 14px', fontSize: '13px', color: 'var(--success)', marginBottom: '18px' }}>
                {successMessage}
              </div>
            )}
            ...
            <button type="submit" disabled={loading} style={{ width: '100%', background: 'var(--navy)', color: '#fff', border: 'none', borderRadius: '10px', padding: '13px', fontSize: '15px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', opacity: loading ? 0.75 : 1, transition: 'opacity var(--transition)' }}>
